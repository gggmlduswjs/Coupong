# 사용 가이드

#guide #usage #howto

**작성일:** 2026-02-05

---

## 🚀 빠른 시작 (5분)

### 1단계: 프로젝트 이동

```bash
cd C:\Users\MSI\Desktop\Coupong
```

### 2단계: DB 초기화 (최초 1회)

```bash
python scripts/init_db_v2_clean.py
```

출력:
```
[OK] 데이터베이스 초기화 완료
[OK] 24개 출판사 등록 완료
[OK] 테스트 계정 생성 완료
```

### 3단계: 전체 워크플로우 실행

```bash
python scripts/smart_upload_system.py
```

### 4단계: 생성된 CSV 확인

```
output/
  ├── coupang_upload_account_1.csv
  ├── coupang_upload_account_2.csv
  ├── coupang_upload_account_3.csv
  ├── coupang_upload_account_4.csv
  └── coupang_upload_account_5.csv
```

---

## 📖 상세 가이드

### 시나리오 1: 특정 키워드로 도서 검색 및 업로드

```python
# example_search.py

from crawlers.aladin_api_crawler import AladinAPICrawler
from analyzers.margin_calculator import MarginCalculator
from analyzers.bundle_generator import BundleGenerator
from scripts.smart_upload_system import SmartUploadSystem
from app.database import get_db

# 1. 알라딘 검색
crawler = AladinAPICrawler()
books = crawler.search_books(
    query="수능완성",
    max_results=100
)
print(f"검색 결과: {len(books)}권")

# 2. DB 저장
db = next(get_db())
for book_data in books:
    book = Book(**book_data)
    db.add(book)
db.commit()

# 3. 마진 분석
calculator = MarginCalculator(db)
products = calculator.analyze_all_books()
print(f"분석 완료: {len(products)}개 상품")

# 4. 묶음 생성
generator = BundleGenerator(db)
bundles = generator.auto_generate_bundles(min_margin=2000)
print(f"묶음 생성: {len(bundles)}개")

# 5. 계정 분산
system = SmartUploadSystem(db)
system.distribute_to_accounts(products + bundles)
print("계정 분산 완료")

# 6. CSV 생성
from exporters.csv_generator import CSVGenerator
exporter = CSVGenerator(db)
csv_files = exporter.generate_all_account_csvs()
print(f"CSV 생성: {len(csv_files)}개 파일")
```

---

### 시나리오 2: 특정 출판사만 검색

```python
# example_publisher.py

from app.database import get_db
from app.models import Publisher, Book

db = next(get_db())

# EBS 출판사만 검색
ebs = db.query(Publisher).filter(
    Publisher.name.in_(["EBS", "한국교육방송공사"])
).all()

print(f"EBS 출판사: {len(ebs)}개")
for pub in ebs:
    print(f"- {pub.name}: 마진율 {pub.margin_rate}%")

# EBS 도서 검색
crawler = AladinAPICrawler()
ebs_books = crawler.search_books(query="EBS 수능", max_results=50)

print(f"EBS 도서: {len(ebs_books)}권")
```

---

### 시나리오 3: 마진 분석만 실행

```python
# example_margin_analysis.py

from analyzers.margin_calculator import MarginCalculator
from app.database import get_db
from app.models import Book, Publisher

db = next(get_db())

# 미분석 도서 조회
unprocessed_books = db.query(Book).filter(
    Book.is_processed == False
).all()

print(f"미분석 도서: {len(unprocessed_books)}권")

# 마진 분석
calculator = MarginCalculator(db)
for book in unprocessed_books:
    publisher = db.query(Publisher).filter(
        Publisher.name == book.publisher_name
    ).first()

    if publisher:
        analysis = calculator.analyze_book(book, publisher)
        print(f"{book.title}: 순마진 {analysis['net_margin']:,}원")

        # 상품 생성
        product = Product.create_from_book(book, publisher)
        db.add(product)

        # 처리 완료 마크
        book.is_processed = True

db.commit()
print("마진 분석 완료")
```

---

### 시나리오 4: 묶음 SKU만 생성

```python
# example_bundles.py

from analyzers.bundle_generator import BundleGenerator
from app.database import get_db

db = next(get_db())

# 묶음 생성기 초기화
generator = BundleGenerator(db)

# 묶음 가능한 그룹 찾기
groups = generator.find_bundleable_books(
    min_books=2,  # 최소 2권
    max_books=5   # 최대 5권
)

print(f"묶음 가능 그룹: {len(groups)}개")
for bundle_key, books in groups.items():
    print(f"- {bundle_key}: {len(books)}권")

# 자동 묶음 생성 (순마진 2000원 이상)
bundles = generator.auto_generate_bundles(min_margin=2000)

print(f"\n생성된 묶음: {len(bundles)}개")
for bundle in bundles:
    print(f"- {bundle.bundle_name}")
    print(f"  권수: {bundle.book_count}권")
    print(f"  판매가: {bundle.total_sale_price:,}원")
    print(f"  순마진: {bundle.net_margin:,}원")
    print()
```

---

### 시나리오 5: 계정별 업로드 현황 확인

```python
# example_check_listings.py

from app.database import get_db
from app.models import Account, Listing
from sqlalchemy import func

db = next(get_db())

# 계정별 통계
accounts = db.query(Account).all()

print("계정별 업로드 현황:\n")
for account in accounts:
    # 업로드 개수
    count = db.query(Listing).filter(
        Listing.account_id == account.id
    ).count()

    # 상품 타입별 개수
    single_count = db.query(Listing).filter(
        Listing.account_id == account.id,
        Listing.product_type == 'single'
    ).count()

    bundle_count = db.query(Listing).filter(
        Listing.account_id == account.id,
        Listing.product_type == 'bundle'
    ).count()

    print(f"{account.account_name}:")
    print(f"  총 업로드: {count}개")
    print(f"  - 단권: {single_count}개")
    print(f"  - 묶음: {bundle_count}개")
    print()
```

출력:
```
계정별 업로드 현황:

account_1:
  총 업로드: 18개
  - 단권: 15개
  - 묶음: 3개

account_2:
  총 업로드: 20개
  - 단권: 18개
  - 묶음: 2개

...
```

---

### 시나리오 6: 특정 계정에 수동 할당

```python
# example_manual_assign.py

from app.database import get_db
from app.models import Product, Account, Listing

db = next(get_db())

# account_1에 특정 상품 할당
account = db.query(Account).filter(
    Account.account_name == "account_1"
).first()

# 무료배송 가능한 상품 중 미업로드된 것
available_products = db.query(Product).filter(
    Product.shipping_policy == 'free',
    Product.status == 'ready',
    Product.isbn.notin_(
        db.query(Listing.isbn).filter(
            Listing.account_id == account.id
        )
    )
).limit(10).all()

# 할당
for product in available_products:
    listing = Listing(
        account_id=account.id,
        product_type='single',
        product_id=product.id,
        isbn=product.isbn,
        sale_price=product.sale_price,
        shipping_policy=product.shipping_policy
    )
    db.add(listing)

db.commit()
print(f"{len(available_products)}개 상품 할당 완료")
```

---

### 시나리오 7: CSV 재생성

```python
# example_regenerate_csv.py

from exporters.csv_generator import CSVGenerator
from app.database import get_db

db = next(get_db())

# 전체 계정 CSV 재생성
exporter = CSVGenerator(db)
csv_files = exporter.generate_all_account_csvs()

print(f"생성된 CSV: {len(csv_files)}개")
for csv_file in csv_files:
    print(f"- {csv_file}")

# 특정 계정만 생성
account_1_csv = exporter.generate_account_csv(account_id=1)
print(f"\naccount_1 CSV: {account_1_csv}")
```

---

### 시나리오 8: Obsidian 로깅 활용

```python
# example_with_logging.py

from obsidian_logger import ObsidianLogger
from crawlers.aladin_api_crawler import AladinAPICrawler

# 로거 초기화
logger = ObsidianLogger()

# 작업 시작
logger.log_to_daily("### 알라딘 크롤링 시작", "작업 시작")

# 크롤링
crawler = AladinAPICrawler()
books = crawler.search_books(query="개념원리", max_results=50)

# 로그 기록
logger.log_to_daily(
    f"검색 완료: {len(books)}권",
    "검색 결과"
)

# 기능 로그
logger.log_feature(
    feature_name="알라딘 검색",
    description=f"개념원리 키워드로 {len(books)}권 검색 완료",
    tags=["crawler", "aladin"],
    status="completed"
)

# 기술 문서 생성
logger.log_technical(
    tech_name="검색 결과 통계",
    content=f"""
    ## 검색 통계
    - 키워드: 개념원리
    - 결과: {len(books)}권
    - 평균 가격: {sum(b['list_price'] for b in books) // len(books):,}원
    """,
    tags=["statistics", "search"]
)
```

---

## 🔧 유지보수

### DB 백업

```bash
# SQLite DB 백업
copy coupang.db coupang_backup_20260205.db
```

### DB 초기화 (전체 삭제)

```bash
# 주의: 모든 데이터 삭제됨!
del coupang.db
python scripts/init_db_v2_clean.py
```

### 특정 테이블만 초기화

```python
# example_reset_listings.py

from app.database import get_db, engine
from app.models import Listing

db = next(get_db())

# listings 테이블 전체 삭제
db.query(Listing).delete()
db.commit()

print("listings 테이블 초기화 완료")
```

---

## 🐛 문제 해결

### 문제 1: "Module not found" 에러

```bash
# 경로 확인
import sys
print(sys.path)

# 프로젝트 루트가 없다면 추가
import sys
from pathlib import Path
project_root = Path(__file__).parent.parent
sys.path.insert(0, str(project_root))
```

### 문제 2: DB 제약조건 위반

```python
from sqlalchemy.exc import IntegrityError

try:
    listing = Listing(account_id=1, isbn="9788...")
    db.add(listing)
    db.commit()
except IntegrityError as e:
    db.rollback()
    print(f"중복 업로드 시도: {e}")
```

### 문제 3: API 타임아웃

```python
# 타임아웃 설정
crawler = AladinAPICrawler()
books = crawler.search_books(
    query="수능완성",
    max_results=50,
    timeout=30  # 30초
)
```

### 문제 4: CSV 인코딩 문제

```python
# UTF-8 with BOM (Excel 호환)
import csv

with open('output.csv', 'w', encoding='utf-8-sig', newline='') as f:
    writer = csv.writer(f)
    writer.writerows(data)
```

---

## 📊 성능 팁

### 팁 1: 배치 처리

```python
# Bad: 건별 커밋
for book in books:
    db.add(book)
    db.commit()  # 느림!

# Good: 배치 커밋
for book in books:
    db.add(book)
db.commit()  # 한 번만
```

### 팁 2: 필요한 필드만 조회

```python
# Bad: 전체 조회
books = db.query(Book).all()

# Good: 필요한 컬럼만
book_titles = db.query(Book.title, Book.isbn).all()
```

### 팁 3: 인덱스 활용

```python
# 인덱스가 있는 컬럼으로 검색
books = db.query(Book).filter(
    Book.isbn == "9788..."  # isbn에 인덱스 있음
).first()
```

---

## 🎯 모범 사례

### 1. 항상 try-except 사용

```python
try:
    # 위험한 작업
    db.commit()
except Exception as e:
    db.rollback()
    logger.error(f"에러 발생: {e}")
```

### 2. 로깅 활용

```python
import logging

logger = logging.getLogger(__name__)
logger.info("작업 시작")
logger.error("에러 발생", exc_info=True)
```

### 3. 환경 변수 사용

```python
# .env 파일
ALADIN_TTB_KEY=your_key_here
DB_PATH=coupang.db

# Python
from app.config import settings
ttb_key = settings.aladin_ttb_key
```

### 4. 테스트 작성

```python
def test_margin_calculation():
    publisher = Publisher(supply_rate=0.27)
    result = publisher.calculate_margin(8000)
    assert result['net_margin'] == 2248
```

---

## 🔗 관련 문서

- [[프로젝트-개요]] - 프로젝트 전체 개요
- [[시스템-아키텍처]] - 시스템 구조
- [[Database-Schema-V2]] - DB 스키마
- [[Tech-Stack]] - 기술 스택

---

## 📞 추가 도움

- Obsidian 일일 노트에 질문 기록
- Claude Code에게 질문
- 관련 문서 참조

---

**업데이트:** 2026-02-05
**상태:** 사용 가능
