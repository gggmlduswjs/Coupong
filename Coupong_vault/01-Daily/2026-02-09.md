...
## 21:55 - 발주서 로직 간소화 (상품명 원본 사용)

- **작업 내용**: 발주서 생성 시 `clean_book_name()`, `split_set_name()` 제거 → 옵션명 첫 콤마 구간 원본 그대로 사용. 거래처별 시트 컬럼을 도서명|주문수량 2열로 간소화 (출판사 컬럼 제거)
- **변경 파일**: `scripts/export_order_sheets.py`, `dashboard.py`
- **이유**: 세트 분리 결과가 부정확("2-2", "완자 1" 등)하여 상품명 원본 그대로 사용하는 것이 더 실용적
