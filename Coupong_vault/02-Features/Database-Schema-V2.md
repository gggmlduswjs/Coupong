...
|------|------|------|
| id | Integer PK | |
| account_name | String UNIQUE | 계정명 (예: 007-book) |
| login_id | String | 쿠팡 로그인 ID |
| email | String | 이메일 |
| password_encrypted | String | 암호화된 비밀번호 |
| vendor_id | String(20) | WING API vendor ID (예: A01105984) |
| wing_access_key | String(100) | WING API access key |
| wing_secret_key | String(100) | WING API secret key |
| wing_api_enabled | Boolean | WING API 활성 여부 |
| outbound_shipping_code | String(50) | 출고지 코드 |
| return_center_code | String(50) | 반품지 코드 |

- 5개 계정: 007-book, 007-ez, 007-bm, 002-bm, big6ceo
- `has_wing_api` property: vendor_id + keys + enabled 모두 있으면 True
- WING API 키 만료: 2026.08.04
- No Alembic: 새 컬럼은 `_migrate_account_columns()`로 ALTER TABLE 추가
