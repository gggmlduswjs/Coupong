# 설정 가이드

#guide #configuration #setup

**작성일:** 2026-02-05

---

## 🔧 초기 설정

### 1. 환경 변수 (.env)

프로젝트 루트에 `.env` 파일 생성:

```bash
# C:\Users\MSI\Desktop\Coupong\.env

# 알라딘 API
ALADIN_TTB_KEY=your_ttb_key_here

# 데이터베이스
DATABASE_URL=sqlite:///coupang_auto.db

# Obsidian
OBSIDIAN_VAULT_PATH=C:\Users\MSI\Desktop\Coupong\obsidian_vault

# 계정 설정
DEFAULT_DAILY_UPLOAD_LIMIT=20
NUM_ACCOUNTS=5

# ── WING API 키 (5계정 각각) ──
# 계정명_VENDOR_ID / 계정명_ACCESS_KEY / 계정명_SECRET_KEY
007_BOOK_VENDOR_ID=A01105984
007_BOOK_ACCESS_KEY=your_access_key
007_BOOK_SECRET_KEY=your_secret_key

007_BM_VENDOR_ID=A00317195
007_BM_ACCESS_KEY=your_access_key
007_BM_SECRET_KEY=your_secret_key

007_EZ_VENDOR_ID=A01234216
007_EZ_ACCESS_KEY=your_access_key
007_EZ_SECRET_KEY=your_secret_key

002_BM_VENDOR_ID=A01163064
002_BM_ACCESS_KEY=your_access_key
002_BM_SECRET_KEY=your_secret_key

BIG6CEO_VENDOR_ID=A01258837
BIG6CEO_ACCESS_KEY=your_access_key
BIG6CEO_SECRET_KEY=your_secret_key
```

> **중요:** WING API 키는 2026.08.04에 만료됩니다. 갱신 필요.
> 환경변수명 매핑은 `app/constants.py`의 `WING_ACCOUNT_ENV_MAP` 참조.

### 2. 알라딘 API Key 발급

1. **알라딘 회원가입**
   - https://www.aladin.co.kr/ 접속
   - 회원가입 (무료)

2. **API Key 신청**
   - https://www.aladin.co.kr/ttb/wstart.aspx
   - TTBKey 발급 신청
   - 승인까지 1-2일 소요

3. **.env에 Key 추가**
   ```
   ALADIN_TTB_KEY=ttb여기에발급받은키입력
   ```

### 3. Python 패키지 설치

```bash
pip install -r requirements.txt
```

필수 패키지:
```
SQLAlchemy==2.0.23
pydantic==2.5.0
pydantic-settings==2.1.0
requests==2.31.0
python-dotenv==1.0.0
```

---

## 📁 디렉토리 구조 설정

### 자동 생성 (권장)

```python
# setup_directories.py

from pathlib import Path

# 필요한 디렉토리 생성
directories = [
    "output",
    "logs",
    "backups",
    "data/raw",
    "data/processed",
]

for dir_path in directories:
    Path(dir_path).mkdir(parents=True, exist_ok=True)
    print(f"[OK] {dir_path}")
```

실행:
```bash
python setup_directories.py
```

### 수동 생성

```
C:\Users\MSI\Desktop\Coupong\
├── output/          (CSV 출력 파일)
├── logs/            (로그 파일)
├── backups/         (DB 백업)
├── data/
│   ├── raw/         (원본 데이터)
│   └── processed/   (가공 데이터)
```

---

## 🔌 WING API 설정 (Phase 2)

### 1. WING Open API 키 발급

```
1. 쿠팡 판매자센터 → WING 관리 → Open API
2. Access Key / Secret Key 발급
3. 각 계정별로 별도 발급 필요 (5개 계정 × 2 키)
```

### 2. 출고지/반품지 코드 조회

```bash
# 5계정 출고지/반품지 코드 자동 조회 + DB 저장
python scripts/setup_shipping_places.py

# 카테고리 확인 포함
python scripts/setup_shipping_places.py --check-category
```

결과 (DB accounts 테이블에 자동 저장):
```
007-book: 출고지=24105055, 반품지=1002509459
007-bm:   출고지=3818765,  반품지=1002504253
007-ez:   출고지=21009623, 반품지=1002504172
002-bm:   출고지=20884668, 반품지=1002504202
big6ceo:  출고지=21307952, 반품지=1002514534
```

### 3. API 연결 테스트

```bash
# WING API 연결 테스트
python scripts/test_wing_api.py

# 상품 등록 테스트 (dry-run)
python scripts/test_api_upload.py --dry-run

# 실제 등록 테스트
python scripts/test_api_upload.py --account 007-book --isbn 9788961334839
```

---

## 🗄️ 데이터베이스 설정

### 초기화

```bash
python scripts/init_db_v2_clean.py
```

출력:
```
[OK] 데이터베이스 생성
[OK] 테이블 생성 (8개)
[OK] 출판사 데이터 등록 (24개)
[OK] 테스트 계정 생성 (5개)
```

### 커스텀 초기화

```python
# custom_init.py

from app.database import engine, get_db
from app.models import Base, Publisher, Account

# 테이블 생성
Base.metadata.create_all(bind=engine)

db = next(get_db())

# 출판사 추가
publishers = [
    Publisher(name="EBS", margin_rate=73, supply_rate=0.27, min_free_shipping=50800),
    Publisher(name="개념원리", margin_rate=65, supply_rate=0.35, min_free_shipping=23900),
    # ... 더 추가
]

for pub in publishers:
    db.add(pub)

# 계정 추가
for i in range(1, 6):
    account = Account(
        account_name=f"account_{i}",
        coupang_id=f"coupang_account_{i}",
        encrypted_password="encrypted",
        daily_upload_limit=20
    )
    db.add(account)

db.commit()
```

---

## 🏢 출판사 설정

### 출판사 데이터 구조

```python
# config/publishers.py

PUBLISHERS = {
    "EBS": {
        "margin_rate": 73,      # 매입율 (%)
        "supply_rate": 0.27,    # 공급률
        "min_free_shipping": 50800,  # 무료배송 기준 (원)
    },
    "한국교육방송공사": {
        "margin_rate": 73,
        "supply_rate": 0.27,
        "min_free_shipping": 50800,
    },
    "개념원리": {
        "margin_rate": 65,
        "supply_rate": 0.35,
        "min_free_shipping": 23900,
    },
    # ... 24개 출판사
}
```

### 출판사 추가

```python
# add_publisher.py

from app.database import get_db
from app.models import Publisher

db = next(get_db())

# 새 출판사 추가
new_publisher = Publisher(
    name="새출판사",
    margin_rate=60,
    supply_rate=0.40,
    min_free_shipping=18000,
    notes="IT 도서 전문 출판사"
)

db.add(new_publisher)
db.commit()

print(f"출판사 추가: {new_publisher.name}")
```

### 출판사 수정

```python
# update_publisher.py

from app.database import get_db
from app.models import Publisher

db = next(get_db())

# 출판사 조회
publisher = db.query(Publisher).filter(
    Publisher.name == "개념원리"
).first()

# 마진율 수정
publisher.margin_rate = 66  # 65% → 66%
publisher.supply_rate = 0.34  # 0.35 → 0.34

db.commit()
print(f"출판사 수정: {publisher.name}")
```

---

## 👤 계정 설정

### 계정 추가

```python
# add_account.py

from app.database import get_db
from app.models import Account

db = next(get_db())

# 새 계정 추가
new_account = Account(
    account_name="account_6",
    coupang_id="your_coupang_id",
    encrypted_password="encrypted_password",
    daily_upload_limit=30,  # 일일 업로드 한도
    is_active=True
)

db.add(new_account)
db.commit()

print(f"계정 추가: {new_account.account_name}")
```

### 계정 활성화/비활성화

```python
# toggle_account.py

from app.database import get_db
from app.models import Account

db = next(get_db())

# 계정 조회
account = db.query(Account).filter(
    Account.account_name == "account_3"
).first()

# 비활성화
account.is_active = False
db.commit()

print(f"계정 비활성화: {account.account_name}")
```

### 일일 한도 조정

```python
# adjust_limit.py

from app.database import get_db
from app.models import Account

db = next(get_db())

# 모든 계정의 일일 한도를 30으로 변경
accounts = db.query(Account).all()

for account in accounts:
    account.daily_upload_limit = 30
    print(f"{account.account_name}: {account.daily_upload_limit}")

db.commit()
```

---

## 🎯 마진 계산 설정

### 기본 설정

```python
# app/config.py

class Settings(BaseSettings):
    # 도서정가제
    BOOK_DISCOUNT_RATE: float = 0.9  # 최대 할인율 10%

    # 쿠팡 수수료
    COUPANG_FEE_RATE: float = 0.11  # 11%

    # 배송비
    DEFAULT_SHIPPING_COST: int = 2000  # 2000원

    # 마진 기준
    MIN_MARGIN_FOR_FREE_SHIPPING: int = 2000  # 무료배송 최소 순마진
    MIN_MARGIN_FOR_PAID_SHIPPING: int = 0     # 유료배송 최소 순마진
```

### 커스텀 마진 계산

```python
# custom_margin.py

class CustomMarginCalculator:
    def __init__(self, shipping_cost=2500):  # 배송비 2500원
        self.shipping_cost = shipping_cost

    def calculate(self, list_price, supply_rate):
        sale_price = int(list_price * 0.9)
        supply_cost = int(list_price * supply_rate)
        coupang_fee = int(sale_price * 0.11)
        margin = sale_price - supply_cost - coupang_fee
        net_margin = margin - self.shipping_cost

        return {
            'sale_price': sale_price,
            'margin': margin,
            'net_margin': net_margin
        }
```

---

## 📦 묶음 설정

### 묶음 생성 조건

```python
# app/config.py

class BundleSettings(BaseSettings):
    MIN_BOOKS_PER_BUNDLE: int = 2    # 최소 권수
    MAX_BOOKS_PER_BUNDLE: int = 10   # 최대 권수
    MIN_BUNDLE_MARGIN: int = 2000    # 최소 순마진

    # 묶음 제외 출판사
    EXCLUDE_PUBLISHERS: list = [
        "마린북스",  # 마진율이 너무 낮음
        "아카데미소프트"
    ]
```

### 묶음 전략 커스터마이징

```python
# custom_bundle_strategy.py

class SmartBundleStrategy:
    def determine_bundle_size(self, books, target_margin=5000):
        """목표 순마진을 맞추기 위한 최적 권수 계산"""
        sorted_books = sorted(books, key=lambda b: b.list_price, reverse=True)

        for i in range(2, min(len(sorted_books) + 1, 11)):
            bundle_books = sorted_books[:i]
            total_margin = self.calculate_bundle_margin(bundle_books)

            if total_margin >= target_margin:
                return bundle_books

        return sorted_books[:5]  # 기본 5권
```

---

## 📤 CSV 출력 설정

### CSV 포맷 설정

```python
# app/config.py

class CSVSettings(BaseSettings):
    # 인코딩
    CSV_ENCODING: str = "utf-8-sig"  # Excel 호환

    # 구분자
    CSV_DELIMITER: str = ","

    # 헤더
    CSV_HEADERS: list = [
        "ISBN",
        "제목",
        "판매가",
        "배송비",
        "배송정책",
        "재고",
        "상품설명"
    ]

    # 출력 경로
    OUTPUT_DIR: str = "output"
```

### 커스텀 CSV 포맷

```python
# custom_csv_format.py

class CustomCSVExporter:
    def format_row(self, listing):
        return [
            listing.isbn,
            listing.title,
            f"{listing.sale_price:,}",  # 천단위 구분
            "0" if listing.shipping_policy == 'free' else "2000",
            "무료배송" if listing.shipping_policy == 'free' else "유료배송",
            "999",  # 재고
            self.generate_description(listing)
        ]

    def generate_description(self, listing):
        return f"""
        {listing.title}

        ■ 정가: {listing.list_price:,}원
        ■ 판매가: {listing.sale_price:,}원 (10% 할인)
        ■ 출판사: {listing.publisher_name}
        ■ 배송: {listing.shipping_policy}

        [정품 새책 / 빠른배송]
        """
```

---

## 📝 로깅 설정

### 기본 로깅

```python
# app/config.py

import logging

# 로깅 설정
logging.basicConfig(
    level=logging.INFO,
    format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
    handlers=[
        logging.FileHandler('logs/app.log'),
        logging.StreamHandler()
    ]
)
```

### Obsidian 로깅 설정

```python
# app/config.py

class ObsidianSettings(BaseSettings):
    VAULT_PATH: str = "C:\\Users\\MSI\\Desktop\\Coupong\\obsidian_vault"

    # 디렉토리 구조
    DAILY_DIR: str = "01-Daily"
    FEATURES_DIR: str = "02-Features"
    TECHNICAL_DIR: str = "03-Technical"
    DECISIONS_DIR: str = "04-Decisions"
    BUGS_DIR: str = "05-Bugs"

    # 로깅 활성화
    ENABLE_OBSIDIAN_LOGGING: bool = True
```

---

## 🔄 스케줄러 설정 (선택사항)

### 일일 자동 크롤링

```python
# scheduler.py

import schedule
import time
from crawlers.aladin_api_crawler import AladinAPICrawler
from scripts.smart_upload_system import SmartUploadSystem

def daily_crawl_job():
    """매일 자동 크롤링 및 업로드"""
    print("일일 크롤링 시작...")

    # 알라딘 검색
    crawler = AladinAPICrawler()
    books = crawler.search_books(query="신간", max_results=100)

    # 전체 워크플로우 실행
    system = SmartUploadSystem()
    system.run_full_workflow()

    print("일일 크롤링 완료!")

# 매일 오전 9시 실행
schedule.every().day.at("09:00").do(daily_crawl_job)

# 스케줄러 실행
while True:
    schedule.run_pending()
    time.sleep(60)  # 1분마다 체크
```

실행:
```bash
python scheduler.py
```

---

## 🚨 알림 설정 (선택사항)

### 이메일 알림

```python
# notifications/email_notifier.py

import smtplib
from email.mime.text import MIMEText

class EmailNotifier:
    def __init__(self, smtp_server, username, password):
        self.smtp_server = smtp_server
        self.username = username
        self.password = password

    def send(self, to_email, subject, body):
        msg = MIMEText(body)
        msg['Subject'] = subject
        msg['From'] = self.username
        msg['To'] = to_email

        with smtplib.SMTP(self.smtp_server, 587) as server:
            server.starttls()
            server.login(self.username, self.password)
            server.send_message(msg)

# 사용
notifier = EmailNotifier(
    smtp_server="smtp.gmail.com",
    username="your@email.com",
    password="your_password"
)

notifier.send(
    to_email="your@email.com",
    subject="쿠팡 업로드 완료",
    body="오늘 20개 상품 업로드 완료!"
)
```

---

## 🔒 보안 설정

### 비밀번호 암호화

```python
# utils/security.py

from cryptography.fernet import Fernet

class PasswordEncryptor:
    def __init__(self, key=None):
        self.key = key or Fernet.generate_key()
        self.cipher = Fernet(self.key)

    def encrypt(self, password: str) -> str:
        return self.cipher.encrypt(password.encode()).decode()

    def decrypt(self, encrypted: str) -> str:
        return self.cipher.decrypt(encrypted.encode()).decode()

# 사용
encryptor = PasswordEncryptor()
encrypted_pw = encryptor.encrypt("my_password")
print(f"암호화: {encrypted_pw}")
```

### API Key 보호

```python
# .env 파일에만 저장 (Git에 커밋 금지!)
# .gitignore에 추가:
.env
*.db
output/
logs/
```

---

## 🔗 관련 문서

- [[프로젝트-개요]] - 프로젝트 전체 개요
- [[사용-가이드]] - 사용법
- [[시스템-아키텍처]] - 시스템 구조
- [[Tech-Stack]] - 기술 스택

---

**업데이트:** 2026-02-05
**상태:** 설정 완료
