...
## 🔌 External API: 쿠팡 WING Open API (Phase 2)

### API 정보
- **제공자:** 쿠팡
- **Base URL:** `https://api-gateway.coupang.com`
- **인증:** HMAC-SHA256 (CEA 방식)
- **Rate Limit:** 10 calls/sec (0.1초 간격)
- **API 키 만료:** 2026.08.04
- **계정:** 5개 (각각 별도 access/secret key)

### 인증 방식
```python
# HMAC-SHA256 서명 생성
message = f"{datetime}{method}{path}{query}"
signature = hmac.new(secret_key, message, hashlib.sha256).hexdigest()
header = f"CEA algorithm=HmacSHA256, access-key={access_key}, signed-date={dt}, signature={sig}"
```
...
| 카테고리 추천 | GET | `/v2/providers/seller_api/apis/api/v1/marketplace/meta/category-related-metas/predict` |
| 카테고리 메타 | GET | `/v2/.../display-category-codes/{code}` |
| 출고지 조회 | GET | `/v2/providers/openapi/apis/api/v1/outbound/shipping-place/...` |
| 반품지 조회 | GET | `/v2/providers/openapi/apis/api/v4/vendors/{vendorId}/returnShippingCenters` |

### 구현 파일
```
app/api/coupang_wing_client.py  - API 클라이언트 (HMAC 인증)
uploaders/coupang_api_uploader.py - 상품 등록 (JSON 빌드)
app/constants.py - BOOK_PRODUCT_DEFAULTS, WING_ACCOUNT_ENV_MAP
```

### 도서 상품 등록 핵심 필드
- `notices`: `noticeCategoryName = "서적"` (NOT "도서"), 7개 항목 개별 객체
- `attributes`: 학습과목, 사용학년/단계, ISBN 필수
- `outboundShippingTimeDay`: item 레벨 (product 레벨 아님)
- `pccNeeded`: boolean `false` (문자열 아님)
- `displayCategoryCode`, `outboundShippingPlaceCode`: 정수형
